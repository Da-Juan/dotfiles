...
KUBE_PROMPT_ENABLED=on
KUBE_PROMPT_DISABLE_PATH="${HOME}/.kube/kube-prompt/disabled"

[[ -f "${KUBE_PROMPT_DISABLE_PATH}" ]] && KUBE_PROMPT_ENABLED=off
...
kubeoff() {
  if [[ "${1}" == '-h' || "${1}" == '--help' ]]; then
    ...
    mkdir -p -- "$(dirname "${KUBE_PROMPT_DISABLE_PATH}")"
    touch -- "${KUBE_PROMPT_DISABLE_PATH}"
  elif [[ $# -ne 0 ]]; then
