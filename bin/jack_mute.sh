...
PULSEUSER="$(ps -ef | /bin/grep "[p]ulseaudio" | tail -1 | /usr/bin/awk '{print $1}')"
...
mapfile -t SINKS <<< "$(/bin/su "$PULSEUSER" /bin/bash -c "PULSE_RUNTIME_PATH=$PULSE_RUNTIME_PATH /usr/bin/pacmd list-sinks" | /bin/sed -n -E 's/^\s+\*?\s+index:\s([0-9]+)/\1/p')"
for i in "${SINKS[@]}"; do
